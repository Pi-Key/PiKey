...
struct NumAndCode identifier(Token** token_array, int current) {
  struct NumAndCode result;

  Token next = *token_array[current + 1];
  if (
      next.type != EQUAL &&
      next.type != PLUS_EQUAL &&
      next.type != MINUS_EQUAL &&
      next.type != LEFT_PARENTHESE
    ) {
    pk_error(formatter("[%d] Unexpected identifier.", next.line));
    exit(1);
  }

  result.resulting_code = formatter("%s ", token_array[current]);
...
struct NumAndCode transpile_token(Token** token_array, int i) {
  ...
  struct NumAndCode final_i_and_code = {i, resulting_code};
